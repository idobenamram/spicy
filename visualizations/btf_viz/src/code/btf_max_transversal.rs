...
fn try_augmenting_path(
    ...
    recorder.push_array_step(line!() - 1, "column_stack", head as usize, &column_stack[head as usize]);
    ...
    while head >= 0 {
        ...
        if visited[col] != current_column {
            ...
            if found {
                ...
                recorder.push_array_step(line!() - 1, "row_stack", head as usize, &row_stack[head as usize]);
                break;
            }
            position_stack[head as usize] = m.col_start(col);
            recorder.push_array_step(line!() - 1, "position_stack", head as usize, &position_stack[head as usize]);
        }
    ...
    if found {
        ...
        while head >= 0 {
            ...
            recorder.push_array_step(line!() - 1, "column_permutations", row, &column_permutations[row]);
            head -= 1;
            recorder.push_number_step(line!() - 1 , "head", &head);
        }
    }
